...
def main() -> None:
    ...
    print("  HUD shows: fps  steps  str(ength)  fb(feedback)  lora")
    print("=" * 60 + "\n")
    ...
    try:
        while True:
            ...
            h, w = clean_out.shape[:2]
            out_frame = np.zeros((h + TOP_BAR + BOT_BAR, w, 3), dtype=np.uint8)
            out_frame[TOP_BAR:TOP_BAR + h, :] = clean_out
            ...
            cv2.putText(out_frame, "+/- steps   [ ] strength   ,/. feedback   Q quit",
                        (8, 34), cv2.FONT_HERSHEY_SIMPLEX, 0.40, (215, 215, 215), 1, cv2.LINE_AA)
            disp = active_prompt if len(active_prompt) <= 52 else active_prompt[:49] + "..."
